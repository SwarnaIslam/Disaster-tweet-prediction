...
class DisaterPredictionSystem:
    ...
    def cleanTweet(self, txt):
        ...
        words = ' '.join([self.lemma.lemmatize(token, self.penn_to_wordnet(pos)) for token, pos in pos_tags if token not in (self.stop)])
        text = "".join(words)
        txt = re.sub('[^a-z]', ' ', text)
        return txt
    ...
    def predict(self, description):
        self.df['cleaned_tweets'] = self.df['text'].apply(self.remove_links)
        self.df['cleaned_tweets'] = self.df['cleaned_tweets'].apply(self.cleanTweet)
        self.df['cleaned_tweets'] = self.df['cleaned_tweets'].apply(self.remove_ne)

        print(self.df.head(10))
        from wordcloud import WordCloud

        plt.figure(figsize=(20, 20))  # Text that is Disaster tweets
        wc = WordCloud(max_words=1000, width=1600, height=800).generate(" ".join(self.df[self.df['target'] == 1].cleaned_tweets))
        plt.imshow(wc, interpolation='bilinear')

        y = self.df.target
        x = self.df.cleaned_tweets
        ...
        for text, label in zip(sentences, predictions):
            if label == 1:
                ...
                # print("text:", text, "\nClass:", target)
                answers.append({"Class": target, "text": text})
                # print()
            else:
                target = "Normal Tweet"
                answers.append({"Class": target, "text": text})
